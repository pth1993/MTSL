...
class BaseModel(nn.Module):
    ...
    def loss(self, input_word, input_char, target, target_fw, target_bw, mask, hx=None, leading_symbolic=0):
        # output from rnn [batch, length, tag_space]
        if self.use_lm:
            ...
            lm_size = lm_fw.size()
            lm_size = (lm_size[0] * lm_size[1], lm_size[2])
            lm_fw = lm_fw.view(lm_size)
            lm_bw = lm_bw.view(lm_size)
            max_len = length.max()
        ...
        if self.use_crf:
            if self.use_lm:
                return self.crf.loss(output, target, mask=mask).mean() + self.lm_loss * (fw_loss + bw_loss)
            else:
    ...
    def decode(self, input_word, input_char, target, mask, hx=None, leading_symbolic=0):
        # output from rnn [batch, length, tag_space]
        if self.use_lm:
            output, _, mask, length, lm_fw, lm_bw = self._get_rnn_output(input_word, input_char, mask, hx=hx)
        else:
